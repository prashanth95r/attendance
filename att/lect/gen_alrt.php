...
$err_flag=false;
$err_msg=array();
$bin_type=pack("H*",$_GET['to_type']);
$to_type=mcrypt_decrypt(MCRYPT_3DES,KEY,$bin_type,"ecb");
if(isset($_POST['submit']))
{
	...
	{
		$err_flag=true;
		$err_msg[]="Body of alert cannot be empty";
	}
	$from=$_SESSION['SESS_FIRST_NAME'];
		if(!$err_flag)
		{
				if($to_type==1)
				{
					$query="INSERT INTO alert_section (alert_by,alert_to_type,alert_to,alert_subject,alert_details) VALUES ('$from','$to_type','ALL HODS','$subject','$alert')";
					$result=mysql_query($query);
					if($result)
					{
						header("location:alrt_sucess.php?id=1");
					}
					else
					{
						echo "failed";
				...
				elseif($to_type==2)
				{
					...
					$alert_to=$_GET['fac_id'];
					$query="INSERT INTO alert_section (alert_by,alert_to_type,alert_to,alert_subject,alert_details) VALUES ('$from','$to_type','$alert_to','$subject','$alert')";
					$result=mysql_query($query);
					if($result)
					{
						header("location:alrt_sucess.php?id=1");
				...
				elseif($to_type==3)
				{
					$query="INSERT INTO alert_section (alert_by,alert_to_type,alert_to,alert_subject,alert_details) VALUES ('$from','$to_type','ALL FACULTY','$subject','$alert')";
					$result=mysql_query($query);
					if($result)
					{
						header("location:alrt_sucess.php?id=1");
				...
				elseif($to_type==6)
				{
					$query="INSERT INTO alert_section (alert_by,alert_to_type,alert_to,alert_subject,alert_details) VALUES ('$from','$to_type','ALL STUDENTS','$subject','$alert')";
					$result=mysql_query($query);
					if($result)
					{
						header("location:alrt_sucess.php?id=1");
				...
				elseif($to_type==8)
				{
					if(!isset($_GET['year'])||$_GET['section'])
					{
						exit();
					}
					$year=$_GET['year'];
					$section=$_GET['section'];
					$alert_to=$year.",".$section;
					$query="INSERT INTO alert_section (alert_by,alert_to_type,alert_to,alert_subject,alert_details) VALUES ('$from','$to_type','$alert_to','$subject','$alert')";
					$result=mysql_query($query);
					if($result)
					{
...
<center><form id="form1" name="form1" method="post" action="">
  <table width="635" border="0">
    <tr>
      <td width="152">Enter Subject</td>
      <td width="473"><label for="subject"></label>
      <input name="subject" type="text" id="subject" value="" size="72" /></td>
    </tr>
    <tr>
      <td>Enter Alert</td>
      <td><label for="alert"></label>
      <textarea name="alert" id="alert" cols="55" rows="15" placeholder="Max 1800 characters"></textarea></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td><input type="submit" name="submit" id="submit" value="Generate Alert" /></td>
    </tr>
  </table>
</form>
