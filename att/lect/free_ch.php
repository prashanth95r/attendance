...
require_once('../funcs/getperiodbyday.php');
require_once('../funcs/checkassign.php');
require_once('../funcs/checkvalidity.php');
if(!isset($_GET['branch'])||!isset($_GET['period']))
{
		exit();
}
	$maxtime =120;
...
<?php
	...
	if($day=="Sunday")
	{
		echo "No schedule for sunday";
		exit();
	}
	$day_id=getdayid2($day);
	$branch=$_GET['branch'];
	$period=$_GET['period'];
	$per_id=($period+$day_id)."_yearsecsub";
	if(!($branch=="*"))
	{	
		$query="SELECT * FROM faculty_schedule_yearsecsub WHERE faculty_dept='$branch'";
		$result=mysql_query($query);
		if($result)
		{echo"<table class=\"inner_table1\"><tr><td><b>ID</b></td><td><b>Name</b></td></tr>";
			if(mysql_num_rows($result)>0)
			{
				while($row=mysql_fetch_array($result))
				{
					$fac_id=$row['user_name'];
					if(!isAlreadyAssigned($fac_id,$per_id,date('Y-m-d')))
					{
						$query="SELECT $per_id FROM faculty_schedule_yearsecsub WHERE user_name='$fac_id'";
						...
						if($result1)
						{
							if(mysql_num_rows($result1)>0)
							{
								...
								if($row1[$per_id]=="FREE")
								{
									echo "<tr><td>".$fac_id."</td><td>".getName1($fac_id)."</td></tr>";
								}
							}
							else
							{
								echo "<tr>No faculty is free on the entered slot</tr>";
							}
						}
						else
						{
							echo "<tr>error occured while querying.</tr>";
						}
					}
				}
			}
			...
			{
				echo "<tr>no one exists in this branch</tr>";
			}
	...
	elseif($branch=="*")
	{echo "<table class=\"inner_table1\"><tr><td><b>ID</td><td><b>Name</b></td><td><b>Department Code</b></td></tr>";
		$query="SELECT * FROM faculty_schedule_yearsecsub";
		$result=mysql_query($query);
		if($result)
		{
			if(mysql_num_rows($result)>0)
					...
					if(!isAlreadyAssigned($fac_id,$per_id,date('Y-m-d')))
					{
						...
						if($result1)
						{
							if(mysql_num_rows($result1)>0)
							{
								...
								if($row1[$per_id]=="FREE")
								{
									echo "<tr><td>".$fac_id."</td><td>".getName($fac_id)."</td><td>".getDeptCode($fac_id)."</td><td></tr>";
								}
							}
							else
							{
								echo "<tr>no rows returned</tr>";
							}
						}
						else
						{
