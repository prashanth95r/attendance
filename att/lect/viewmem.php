...
function getpercent($subject,$roll)
{
...
$dayt=date($day,strtotime("$day -1day"));

$qur7="SELECT * FROM att_table WHERE roll_number=$roll ";
$res7=mysql_query($qur7) or die(mysql_error());
...
{
$day=$row7['date'];
if(strtotime($day)>=strtotime($dayt))
{
if($row7['subject']==$subject)
{
$att=decrypt1($row7['p_or_a'],$day);
...
$qur="SELECT * FROM student_current_details WHERE current_sec='$section' AND current_year='$year' ORDER BY roll_number";
...
$qur1="SELECT * FROM att_table WHERE section='$section' AND roll_number='$roll' AND date>='$day'";
$res1=mysql_query($qur1) or die(mysql_error());
if($res1)
{
$z=0;
$arr=array();
...
if(!empty($arr1))
{
echo "<table class=\"inner_table1\" border=\"1\"><tr><td>roll_no</td>";
$k=0;

for($k=0;$k<=$y;$k++)
{
$name=getFullPeriod($arr1[$k]);
echo "<td colspan=\"2\"><b>".$name."(attended/total)</b></td>";
}
echo "<td colspan=\"2\">total(attended/total)</td>";
echo "<td><b>View Full Attendence</b></td></tr>";
$qur="SELECT * FROM student_current_details WHERE current_sec='$section' AND current_year='$year'";
$res=mysql_query($qur);
while($row=mysql_fetch_array($res))
{
$roll=$row['roll_number'];
echo "<tr><td>".$row['roll_number']."</td>";
$s=0;$s1=0;
for($i=0;$i<=$z;$i++)
{
$a=getpercent($arr[$i],$row['roll_number']);
$s+=$a[0];$s1+=$a[1];
if($a[1])
$per=($a['0']*100)/$a['1'];
else $per=0;
echo "<td>".$a['0']."</td><td>".$a['1']."</td></td>";
}
echo"<td>$s</td><td>$s1</td><td><a href=\"viewatt.php?roll=$roll\">View</a></td>";
...
else 
{
	...
	echo "</script>";
	echo "<h2><center>No details found</center></h2>";
