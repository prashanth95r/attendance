...
<div style=" display: inline-block; width: 100%; background: #474747; padding: 0px 0px 5px 0px;">
     <ul id="menu" style="margin-top: 0px; vertical-align: middle;">
		...
		require_once '../com/connection.php';
		$branch=$_GET['dept_id'];
		$query="SELECT * FROM faculty_login WHERE faculty_dept='$branch'";
		echo"<table class=\"inner_table1\">";
		echo"<tr><td><b>Id no.</b></td><td><b>Name</b></td><td><b>View todays schedule</b></td><td><b>View semester schedule</b></td></tr>";
		...
		if($result)
		{
			if(mysql_num_rows($result)>0)
			{
				while($row=mysql_fetch_assoc($result))
				{
					...
					{
						$x=$row['user_name'];
						echo "<tr><td><h3>".$row['user_name']."</h3></td><td><h3>".$row['name']."</td><td><h3>"."<a href=\"view_fac.php?id=$x \">view</a></h3></td><td><h3><a href=\"sem_sch.php?id=$x \" >view </a>"."</h3></td></tr>";
					}
					elseif($row['user_status']==0)
					{
						echo "<tr><h3>".$row['user_name']."\t".$row['name']."\t"."suspended</h3></tr>";
					}
				}
			}
		}
