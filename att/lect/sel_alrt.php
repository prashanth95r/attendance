...
$query="SELECT * FROM alert_section ORDER BY alert_time DESC LIMIT $sid,$tid";
$result=mysql_query($query);
if($result)
{
	$pqr="UPDATE faculty_login SET alert_time=NOW() WHERE user_name='$username'";
	$res=mysql_query($pqr);
	if(mysql_num_rows($result)>0)
	{
		echo "<table class=\"inner_table1\" width=\"500\" border=\"1\">";
		
		echo "<th>Alert From</th>";
		echo "<th>Time at which alert generated</th>";
		echo "<th>&nbsp;</th>";
	...
	else
	{
		...
		echo "<h2><center>No alerts to display</center></h2>";
	}
	if($userlevel==2)
	{
		while($row=mysql_fetch_array($result))
		{
			...
			{
				$id=mcrypt($row['uni_id']);
				echo "<tr>";
				echo "<td>".getName1($row['alert_by'])."</td>";
				echo "<td>".$row['alert_time']."</td>";
				echo "<td>"."<a href=\"view_alrt2.php?id=$id\">View Alert</a>"."</td>";
				echo "</tr>";
				$sid++;
			}
		...
		echo "</table>";
		if($sid>=29)
	echo "<a href=\"sel_alrt.php?sid=$sid&tid=$tid\">View more alerts</a>";
		
		
	}
	elseif($userlevel==1||$userlevel==3)
	{
		while($row=mysql_fetch_array($result))
		{
