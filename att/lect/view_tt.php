...
if(function_exists('set_time_limit')) set_time_limit($maxtime);
	$x=$_SESSION['SESS_FIRST_NAME'];
	$day=date('l');
	echo"<table class=\"inner_table1\">";
	if($day=="Sunday")
	{
		echo "<tr><td><h1>No schedule for today</h1></td></tr>";
		exit();
	}
	...
	echo "<tr><td><b>Year</b></td><td><b>Section</b></td><td><b>Subject</b></td><td><b>No.of Periods</b></td><td><b>Enter Attendance</b></td></tr>";
	$a=array(1,2,3,4,5,6,7);
	for($i=0;$i<7;$i++)
		$a[$i]=($a[$i]+$day_id)."_yearsecsub";
	$query="SELECT * FROM faculty_schedule_yearsecsub WHERE user_name=$x";
	...
	if($result)
	{
		...
		for($i=0;$i<7;$i++)
		{
			if($row[$a[$i]]!="FREE"&&!empty($row[$a[$i]])&&!existsAssigned2Class($i+1)&&!existsAssignedClass($i+1))
			{
				$x=$i+1;$y=$x+1;
				if($i<5&&($row[$a[$i]]==$row[$a[$x]])&&($row[$a[$i]]==$row[$a[$y]])&&!existsAssigned2Class($x+1)															&&!existsAssignedClass($x+1)&&!existsAssigned2Class($y+1)&&!existsAssignedClass($y+1))
				{
						$flg=0;
						$np=3;
						$sp=$i+1;
						...
						$section=$det[1];
						$subject_id=$det[2];
						$subject=getPeriod($subject_id);
						echo "<tr><td>".$year."</td><td>".$section." </td><td> ".$subject."</td><td>".$np;
						if(!isElective($subject_id))
						{
							if(!isLiveSem($year))
							{
								echo "<td>Semester Ended</td></tr>";
							}
							elseif(!isAlreadyEntered($i+1,$year,$section))
							{
								echo	"<td><a href=\"enter_att.php?flg=$flg&yearsecsub=$x&np=$np&sp=$sp \">Enter attendence</a>"."</td></tr>";
							}
							else
							{
								echo "<td>Entered</td></tr>";
							}
						}
						elseif(isElective($subject_id))
						{
							if(!isLiveSem($year))
							{
								echo "<td>Semester Ended</td></tr>";
							}
							elseif(!isAlreadyEnteredEle($i+1,$subject_id))
							{
				...
				elseif($i<6&&$row[$a[$i]]==$row[$a[$x]]&&!existsAssigned2Class($x+1)&&!existsAssignedClass($x+1))
				{
					$flg=0;
					$np=2;
					$sp=$i+1;
			...
			elseif(existsAssigned2Class($i+1)&&!existsAssignedClass($i+1))
			{
				//"<tr><td>"."-"."</td><td>"."-"." </td><td> "."-"."</td><td>"."-"."</td>"."<td>FREE</td>";
			}
			elseif(existsAssignedClass($i+1)&&!existsAssigned2Class($i+1))
			{
				if(!isLab($i+1)&&!isTwoperiods($i+1))
				{
					$flg=1;
					$np=1;
					$sp=$i+1;
					$det1=existsAssignedClassDet($i+1);
					$det2=getDat($det1[1]);
					$year=$det2[0];
					$section=$det2[1];
					$subject_id=$det2[2];
					$subject=getPeriod($subject_id);
					$x=removeSub($det1[1]);
					echo "<tr><td>".$year."</td>"."<td>".$section."</td>"."<td>-</td>"."<td>".$np;
						if(!isElective($subject_id))
						{
							if(!isLiveSem($year))
							{
				...
				elseif(!isLab($i+1)&&isTwoPeriods($i+1))
				{
					$flg=1;
					$np=2;
					$sp=$i+1;
			...
			elseif(existsAssignedClass($i+1)&&existsAssigned2Class($i+1))
			{
				...
				elseif(isLab($i+1)&&!isTwoPeriods($i+1))
				{
					...
					$subject=getPeriod($subject_id);
					$x=removeSub($det1[1]);
					echo "<tr><td>".$year."</td><td>".$section." </td><td> ".$np;
						if(!isElective($subject_id))
						{
							if(!isLiveSem($year))
							{
