...
<?php
	if(isset($_GET['errid'])&&$_GET['errid']==1)
	{
		...
		echo "alert(\"some error encountered while processing your request.\")";
		echo "</script>";
	}
	if(isset($_POST['submit']))
	{
		...
		else
		{
			...
			$em=$_POST['recemail'];
			$queryent="UPDATE faculty_login SET recemail='$em',recflag='1' WHERE user_name=$username;";
			...
			{
				echo "Sucessfully entered recovery email address.We are redirecting you.";
...
<center><form id="form1" name="form1" method="post" action="">
  <table width="500" border="0" class="inner_table">
    <tr>
      <td>Enter Recovery Email Address</td>
      <td><label for="recemail"></label>
      <input type="text" name="recemail" id="recemail" /></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
...
<h2><center><a href="skip.php">Skip for this time</a></center></h2>
</div></p>
								</div>
							</div>
						</td>
