...
function checkAll(formname, source)
{
	...
	if($result)
	{
		...
		{
			$i=0;
			echo "<center><form action=\"promote.php\" id=\"form1\" name=\"myform\">";
			echo "<input type=\"radio\" name=\"dept\" id=\"year\" value=\"$dept\" checked style=\"display:none\"/>";
			echo "<input type=\"radio\" name=\"year\" id=\"section\" value=\"$year\" checked style=\"display:none\"/ >";
			echo "<input type=\"radio\" name=\"sem\" id=\"period\" value=\"$sem\" checked style=\"display:none\"/></br>";
			
			echo"<table class=\"inner_table1\">";
			echo"<tr><td><input type=\"checkbox\"  onclick=\"javascript:checkAll('myform',this);\"><b>Check All</b></input></td><td><b>Roll No.</b></td><td><b>Name</b></td></tr>";
			while($row=mysql_fetch_array($result))
			{
				if($row['current_sem']==$sem)
				{
					$r[$i]=$row['roll_number'];
					echo "<tr><td><input type=\"checkbox\" value=\"1\" name=\"$r[$i]\"></td><td>".$row['roll_number']."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td>".getNameOfStu($row['roll_number'])."<td></tr><br/>";
					$i++;
				}
			}
			echo "</table>";
		...
		{
			echo "<script type=\"text/javascript\">";
			echo "alert(\"No student exits.\")";
			echo "</script>";
			echo "<h2><center>No student exists</center></h2>";
			header("refresh:2;url=select_year.php");
		}
	}
	else
	{
