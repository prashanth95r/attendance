<?php
	...
	require_once('../funcs/getDat.php');
	require_once('../funcs/electivefuncs.php');
	require_once('../funcs/checkvalidity.php');
	if(!isset($_GET['flg'])||!isset($_GET['yearsecsub'])||!isset($_GET['np'])||!isset($_GET['sp']))
	{
...
<?php
	...
	$date=$_GET['date'];
	$r=array();
	$branch=substr($section,0,(strlen($section)-1)); 
	$sem=getLiveSem($year,$branch);
	$isAlreadyEnteredFlag=false;
	$sppp=$sp;
	$ffff=array();
	if($flag==0)
	{
		...
		for($ppp=0;$ppp<$np;$ppp++)
		{
			if(isAlreadyEntered2($sppp,$year,$section,$date,$subject_id))
			{
				$isAlreadyEnteredFlag=true;	
				$ffff[]=$sppp;
			}
			$sppp=$sp+1;
		}
...
function checkAll(formname, source)
{
	...
	if(!($isAlreadyEnteredFlag)&&isLiveSem($year))
	{
		if($flag==0)
		{
			$i=0;$x=0;
			...
			if($result)
			{
				...
				{
					echo "<center><form action=\"test10.php\" id=\"form1\" name=\"myform\">";
					echo "<input type=\"radio\" name=\"year\" id=\"year\" value=\"$year\" style=\"display:none\" checked/>";
					echo "<input type=\"radio\" name=\"section\" id=\"section\" value=\"$section\" style=\"display:none\" checked/>";
					echo "<input type=\"radio\" name=\"period\" id=\"period\" value=\"$sp\" style=\"display:none\" checked/>";
					echo "<input type=\"radio\" name=\"noperiod\" id=\"noperiod\" value=\"$np\" style=\"display:none\" checked/>";
					echo "<input type=\"radio\" name=\"subject\" id=\"subject\" value=\"$subject_id\" style=\"display:none\" checked/>";
					echo "<input type=\"radio\" name=\"date\" id=\"date\" value=\"$date\" style=\"display:none\" checked/>";
					echo "<br/>";
					while($row=mysql_fetch_array($result))
					{
						if($row['sem_live_flag']==1&&$sem==$row['current_sem'])
						{
							$no_stu=++$x;
							$r[$i]=$row['roll_number'];
							echo "<input type=\"checkbox\" value=\"1\" name=\"$r[$i]\">".substr($row['roll_number'],-3)."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
							$i++;
							if($i%5==0)
							{
								echo "<br/>";
					...
					if($x!=0)
					{
						echo "<br><input type=\"checkbox\"  onclick=\"javascript:checkAll('myform',this);\">Check All</input>";
					}
					if($x==0)
					{
						echo "<h1><center><b>No relevant data found.</b></center></h1>";
						exit();
					}
					echo "<br/><input type=\"submit\" name=\"submit\" id=\"submit\" value=\"Submit\" /></form>";
				}
				else
				{
					echo "<h2><center>Currently no students exists in this section.contact admin for more details</center></h2>";
				}
			}
			else
			{
				echo "<h2><center>No such Section exists.</center></h2>";
			}
		}
		elseif($flag==1)
		{	
				echo "<center><form action=\"test20.php\" id=\"form1\">";
				echo "<input type=\"radio\" name=\"year\" id=\"year\" value=\"$year\" style=\"display:none\" checked/>";
				echo "<input type=\"radio\" name=\"section\" id=\"section\" value=\"$section\" style=\"display:none\" checked/>";
				echo "<input type=\"radio\" name=\"period\" id=\"period\" value=\"$sp\" style=\"display:none\" checked/>";
				echo "<input type=\"radio\" name=\"noperiod\" id=\"noperiod\" value=\"$np\" style=\"display:none\" checked/>"
;				echo "<input type=\"radio\" name=\"date\" id=\"date\" value=\"$date\" style=\"display:none\" checked/></br>";
				echo "<h1><center><b>Enter the subject code of the subject which you taught</b></center></h1>";
				echo "Enter subject code<input type=\"text\" name=\"subject\" id=\"subject\" value=\"\"></br>";
				echo "<input type=\"submit\" name=\"submit\" id=\"submit\" value=\"submit\">";
		}
	}
	elseif($isAlreadyEnteredFlag)
	{
		echo "Already entered attendence for";
		foreach($ffff as $ffff1)
		{
			echo " Period ".$ffff1." ";
		}
	}
	elseif(!isLiveSem($year))
	{
		echo "Semester has been ended";
		header("refresh=3;url=view_tt.php");
	}
?>
