...
if(isset($_POST['submit']))
{
	...
	{
		$err_flag=true;
		$err_msg[]="Year Cannot be empty";
	}
	if(empty($branch))
	{
		$err_flag=true;
		$err_msg[]="Branch Cannot be empty";
	}
	if(empty($section))
	{
		$err_flag=true;
		$err_msg[]="Section Cannot be empty";
	}
	if(!isValidSection($section,$year)&&!empty($section))
	{
		$err_flag=true;
		$err_msg[]="Enter a valid section";
	}
	if(!$err_flag)
	{
		header("location:gen_alrt.php?to_type=$to_type&year=$year&branch=$branch&section=$section");
	}
	else
	{
