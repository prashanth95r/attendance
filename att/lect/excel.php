...
<?php
	$dept=$_SESSION['dept'];
	$query="SELECT * FROM faculty_login WHERE faculty_dept='$dept' AND user_level='1'";
	$result=mysql_query($query);
	if($result)
	{echo "<table class=\"inner_table1\">";
	echo"<tr><td><b>User Id</b></td><td><b>Name</b></td><td><b>Last Name</b></td><td><b>Password</b></td></tr>";
		if(mysql_num_rows($result)>0)
		{
			while($row=mysql_fetch_array($result))
			{
				...
				echo "<td>".ABDecryption($row['user_name'],$row['password'])."</td></tr>";
			}
		
		}
		else
		{
			echo "<tr>No users to display</tr>";
		}
		echo "</table>";
	}
	else
