...
if(isset($_POST['submit']))
{
	...
	if(empty($year)||empty($section))
	{
		...
		{
			$errflag=true;
			$errmsg[]="Section cannot be empty";
		}
	}
	$q="SELECT * FROM student_current_details WHERE current_year='$year' AND current_sec='$section'";
	$r=mysql_query($q);
	if(mysql_num_rows($r)==0&&!empty($section))
	{
			echo "<script type=\"text/javascript\">";
			echo "alert(\"You entered invalid section.\")";
			echo "</script>";	
	}
	elseif($errflag)
	{
	...
	{
		header("location:excel1.php?year=$year&section=$section");
	}
}
?>
<center><form id="form1" name="form1" method="post" action="">
  <table width="416" border="0">
    <tr>
      <td width="199">Enter Year</td>
      <td width="207"><label for="year"></label>
