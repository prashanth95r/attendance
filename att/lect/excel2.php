...
<?php
	...
	$query="SELECT * FROM student_current_details WHERE current_year='$year' AND current_sec='$section'";
	...
	if($result)
	{
		...
		else
		{
			...
			echo "alert(\"No relevant data found.\")";
			echo "</script>";
			echo "<h2><center>No relevant data found.</center></h2>";
			header("refresh:2;url=sel_sec_pwd1.php");
			exit();
		}
	}
	else
	...
	echo "<tr><td><b>Roll no</b></td><td><b>Parent Name</b></td><td><b>Password</b></td></td>";
	for($x=0;$x<$i;$x++)
	{	
		$roll_num=$roll[$x];
		$query="SELECT * FROM student_login WHERE roll_number='$roll_num'";
		$result=mysql_query($query);
		if($result)
		{
			if(mysql_num_rows($result)>0)
			{
				...
				echo "<td>".ABDecryption(substr($row['roll_number'],-9),$row['password'])."</td></tr>";
			}
			
			else
			{
				echo "<tr><td>Unknown error</td></tr>";
			}
		}
		else
		{
			echo "<tr>Failed to execute query</tr>";
		}
	}
	echo "</table>";
?>
