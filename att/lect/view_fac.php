...
<?php
	...
	require_once('../funcs/checkassign1.php');
	require_once('../funcs/checkvalidity.php');
?>
<?php
	if((!$_SESSION['user_level']>=2&&$_SESSION['user_level']<=4))
	{
...
<?php
	...
	echo"<tr><td><b>Year</b></td><td><b>Section</b></td><td><b>Subject</b></td><td><b>Assign this class to other faculty/delete</b></td></tr>";
	for($i=0;$i<7;$i++)
	...
	if($result)
	{
		...
		for($i=0;$i<7;$i++)
		{
			if(!existsAssigned2Class($i+1,$x)&&!existsAssignedClass($i+1,$x))
			{
					$n=$i+1;
					$det=getDat($row[$a[$i]]);
					$year=$det[0];
					...
					echo "<tr><td>".$year."</td><td>".$section."</td><td>".$subject."</td>";
					echo "<td><a href=\"assign_class.php?thisfac_id=$x&per_id=$n \">&nbsp;assign </a></td></tr>";
					}
					else
					{
						echo "<tr><td>-</td><td>-</td><td>-</td><td>".$year."<td></tr>";
					}
			}
			elseif(existsAssigned2Class($i+1,$x)&&!existsAssignedClass($i+1,$x))
				{
					$n=$i+1;
					$year="FREE";
					echo "<tr><td>-</td><td>-</td><td>-</td><td>".$year."<td></tr>";
				...
				elseif(existsAssignedClass($i+1,$x)&&!existsAssigned2Class($i+1,$x))
				{
					$n=$i+1;
					$det1=existsAssignedClassDet($i+1,$x);
					echo "Assigned class of ".sfrm($det1[0])."&nbsp;&nbsp;&nbsp;";
					$det2=getDat($det1[1]);
					$year=$det2[0];
					$section=$det2[1];
					$subject_id=$det2[2];
					$subject=getPeriod($subject_id);
					echo "<tr><td>".$year."</td><td>".$section."</td><td>".$subject."</td>";
					echo "<td><a href=\"dlt_assign_class.php?thisfac_id=$x&per_id=$n\">&nbsp;delete </a></td></tr>";
					
				}
				elseif(existsAssignedClass($i+1,$x)&&existsAssigned2Class($i+1,$x))
				{
					echo "<tr><td>-</td><td>-</td><td>-</td><td>FREE<td></tr>";
				}
		}
	}
?>
