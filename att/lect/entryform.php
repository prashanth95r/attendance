...
$dep=$_SESSION['dept'];
function randomPassword($length = 9, $add_dashes = false, $available_sets = 'luds') 
			{
   				$sets = array();
...
{
		$name=$_POST['fac_name'];
		$sfrm=$_POST['fac_sname'];
		$id=$_POST['ID'];
		if(empty($name)||empty($sfrm)||empty($id))
		{
			echo "<script type=\"text/javascript\">";
			echo "alert(\"All fields are compulsory\")";
			echo "</script>";

		}
		else
		{
			$q="SELECT * FROM faculty_login where user_name='$id'";
			$r=mysql_query($q);
			if(mysql_num_rows($r)==0)
			{
            			$query=mysql_query("INSERT INTO faculty_details (user_name,faculty_dept,faculty_sfrm,faculty_name) VALUES ('$id','$dep','$sfrm','$name') ")or die(mysql_error());	
				$quer1=mysql_query("INSERT INTO faculty_schedule_yearsecsub (user_name,faculty_dept) VALUES ('$id','$dep')")or die(mysql_error());
				$password=randomPassword();
				$roll=$id;
				$password=ABEncryption($roll,$password);
				$quer2=mysql_query("INSERT INTO faculty_login (user_name,name,password) VALUES ('$id','$name','$password')")or die(mysql_error());
				if($query&&$quer1&&$quer2)
				{
					echo "<script type=\"text/javascript\">";
					echo "alert(\"Sucessfully cretaed\")";
					echo "</script>";
					echo "<h2><center>Successfully created faculty</center></h2>";		
				}
				else
				{
					echo "<h2><center>Error in cretaing faculty</center></h2>";
				}
			}
			else	
			{
				echo "<script type=\"text/javascript\">";
				echo "alert(\"User already exists with the entered ID.\")";
				echo "</script>";	
			}
		}
...
<tr><td>Faculty ID </td><td><input type="text" name="ID" /></td></tr>
<tr><td>Faculty Name</td><td><input type="text" name="fac_name" /></td></tr>
<tr><td>Faculty Short Notation</td><td><input type="text" name="fac_sname" /></td></tr>
