...
if(isset($_GET['did']))
{
	if($_GET['did']==0)
	{
		echo "Detained Successfully</br>";
	}
	elseif($_GET['did']==1)
	{
		echo "Failed to detain</br>";
	}
}
?>
<?php
...
$query="SELECT * FROM student_current_details WHERE current_year='$year' AND current_branch='$dept' ORDER BY roll_number";
	...
	if($result)
	{
		if(mysql_num_rows($result)>0)
		{
			...
			echo"<tr><td><b>Roll no.</b></td><td><b>Name</b></td><td><b>Detain</b></td></tr>";
			while($row=mysql_fetch_array($result))
			{
				$roll=$row['roll_number'];
				echo "<tr><td>".$row['roll_number']."</td>&nbsp;&nbsp;&nbsp;&nbsp;";
				echo "<td>".getNameOfStu($row['roll_number'])."</td>&nbsp;&nbsp;&nbsp;&nbsp;";
				echo "<td><a href=\"detain.php?roll=$roll&dept=$dept&year=$year&sem=$sem\">Detain</a>";
				echo "</td></tr>";
			}
		}
		else
		{
			...
			header("refresh:2;url=select_year_det.php");
		}
	}
	else
	{
