...
if($fac_branch=="S&H"&&$year==1)
{
	$no_stu=0;
	$r=array();
	$query="SELECT * FROM student_current_details WHERE current_year='$year' AND current_branch='$dept' ORDER BY roll_number";
	...
	{
		$query2="UPDATE student_current_details SET current_year='$cr',current_sem='1',current_ele1='$ele',current_ele2='$ele',current_ele3='$ele' WHERE roll_number='$pc[$i]'";
		$query4="DELETE FROM att_table WHERE roll_number='$pc[$i]'";
	...
	{
		$query2="UPDATE student_current_details SET current_year='$year',current_sem='1',current_ele1='$ele',current_ele2='$ele',current_ele3='$ele' WHERE roll_number='$npc[$i]'";
		$query4="DELETE FROM att_table WHERE roll_number='$npc[$i]'";
		$result2=mysql_query($query2);
		$result4=mysql_query($query4);
		if(!$result2||!$result4)
		{
	...
	{
		mysql_query("COMMIT");
		header("location:prom_res.php?id=0");
	}
	elseif(!$trans_flag)
	{
		mysql_query("ROLLBACK");
		header("location:prom_res.php?id=1");
	}
}
...
elseif($fac_branch=="MCA")
{
	if($sem==1)
	{
		...
		for($i=0;$i<=$pc_count;$i++)
		{
			$query2="UPDATE student_current_details SET current_year='$cr',current_sem='2',current_ele1='$ele',current_ele2='$ele',current_ele3='$ele' WHERE roll_number='$pc[$i]'";
			$query4="DELETE FROM att_table WHERE roll_number='$pc[$i]'";
			$result2=mysql_query($query2);
			$result4=mysql_query($query4);
			if(!$result2||!$result4)
	...
	elseif($sem==2)
	{
		...
		elseif($year==2)
		{
			...
			$cr=$year+1;
			$ele="NULL";
			$query110="SELECT * FROM student_current_details WHERE current_year='$cr'";
			$result110=mysql_query($query110);
			if($result110)
			{
				while($row110=mysql_fetch_array($result110))
				{
					$roll=$row110['roll_number'];
					$proll="p_".$row110['roll_number'];
					$query5="DELETE FROM student_basic WHERE roll_number='$roll'";
					$query6="DELETE FROM student_login WHERE roll_number='$roll'";
					$query7="DELETE FROM att_table WHERE year='$cr'";
					$query8="DELETE FROM student_login WHERE roll_number='$proll'";
			...
			$query3="DELETE FROM student_current_details WHERE current_year='$cr'";
