...
if(!mysql_num_rows($res)>0)
{
	...
	echo "alert(\"No such details found\")";
	echo "</script>";
	echo "<h2><center>No such details found</center></h2>";
	header("refresh:1;url=studentdetails.php");
	exit();
	
}
echo "<table class=\"inner_table1\" border=\"1\">";
echo "<th>roll no</th><th>name</th><th>view complete details</th>";
while($row=mysql_fetch_array($res))
{
$roll=$row['roll_number'];
echo"<tr><td><h3>$roll</h3></td>";
$qure="SELECT * FROM student_login WHERE roll_number='$roll'";
$res1=mysql_query($qure)or die(mysql_error());
$row1=mysql_fetch_array($res1);
$name=$row1['name'];
echo"<td><h3>$name</h3></td><td><a href=\"admissiondetails.php?roll=$roll\"><h3>view</h3></a></td></tr>";
}
}
else echo"please select valid section";
