...
$period=$_GET['per_id'];
$branch=$_GET['branch'];
$query="SELECT * FROM faculty_details WHERE faculty_dept='$branch'";
	...
	if($result)
	{
		if(mysql_num_rows($result)>0)
		{
			...
			{
				if(!($row['user_name']==$this_faculty))
				{
					$fac_id=$row['user_name'];
					echo "<td>".$row['faculty_name']."</td>";
					echo "<td>"."<a href=\"assign_class.php?thisfac_id=$this_faculty&per_id=$period&fac_id=$fac_id\">Assign to this faculty</a>"."</td>";
					echo "</tr>";
				}
			}
			echo "</table>";
