...
echo "<th>id</th><th>name</th><th>last name</th><th>Make Principal</th>";
while($row=mysql_fetch_array($res))
{
$id=$row['user_name'];
$name=$row['name'];
$lname=$row['last_name'];
echo "<tr><td>$id</td><td>$name</td><td>$lname</td><td><a href=\"makepd.php?val=$id\">principal</a></td></tr>";
}
...
if(isset($_POST['submit']))
{
	$id=mysql_real_escape_string($_POST['id']);
 	$query="SELECT * FROM faculty_login WHERE user_name='$id'";
	$res=mysql_query($query)or die(mysql_error());
	if(mysql_num_rows($res)>0)
	{
		header("location:makepd.php?val=$id");
	}
	else 
	{
		echo "<script type=\"text/javascript\">";
		echo "alert(\"Please enter a valid id\")";
		echo "</script>";
		echo "<h2><center>Please enter a valid id</center></h2>";
	}
}
...
<br><br><br><br><br>
<a href="delp.php"><h3><center>Click here to delete principal</center></h3></a>
