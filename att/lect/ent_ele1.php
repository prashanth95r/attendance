...
else
{
	if(mysql_num_rows($result)>0)
	{
		while($row=mysql_fetch_array($result))
		{
			...
			$roll=substr($row['roll_number'],-5);
			$ele1_name=(($roll*10)+1);
			$ele2_name=(($roll*10)+2);
			$ele3_name=(($roll*10)+3);
			$ele1=strtoupper($_GET[$ele1_name]);
			$ele2=strtoupper($_GET[$ele2_name]);
			$ele3=strtoupper($_GET[$ele3_name]);
			if(empty($ele1)||$ele1=="NULL")
			{
				$check_ele1=true;
				$ele1="NULL";
			}
			...
			{
				$check_ele1=check($ele1);
			}
			
			
			if(empty($ele2)||$ele2=="NULL")
			{
				$check_ele2=true;
				$ele2="NULL";
			}
			...
			{
				$check_ele2=check($ele2);
			}
			
			
			if(empty($ele3)||$ele3=="NULL")
			{
				$check_ele3=true;
				$ele3="NULL";
			}
			...
			if($check_ele1)
			{
				$query2="UPDATE student_current_details SET current_ele1='$ele1' WHERE roll_number='$rollnumber'";
				$result2=mysql_query($query2);
				if(!$result2)
				{
					$err_count++;
					$err_flag=true;
					$err_rollnums[]=$rollnumber;
					$err_msg[]="failed to update elective 1";
				}
			}
			else
			{
					...
					$err_flag=true;
					$err_rollnums[]=$rollnumber;
					$err_msg[]="you entered wrong elective code 1";
			}
			if($check_ele2)
			{
				$query3="UPDATE student_current_details SET current_ele2='$ele2' WHERE roll_number='$rollnumber'";
				...
				if(!$result3)
				{
					...
					$err_flag=true;
					$err_rollnums[]=$rollnumber;
					$err_msg[]="failed to update elective 2";
				}
			}
			else
			{
					...
					$err_flag=true;
					$err_rollnums[]=$rollnumber;
					$err_msg[]="you entered wrong elective code2";
			}
			if($check_ele3)
			{
				$query4="UPDATE student_current_details SET current_ele3='$ele3' WHERE roll_number='$rollnumber'";
				...
				if(!$result4)
				{
					...
					$err_flag=true;
					$err_rollnums[]=$rollnumber;
					$err_msg[]="failed to update elective 3";
				}
			}
			else
			{
				...
				$err_flag=true;
				$err_rollnums[]=$rollnumber;
				$err_msg[]="you entered wrong elective code3";
			}
		}
	}
	else
	{
		echo "no students in this section";
		exit();
	}
	if($trans_flag)
	{
	...
	else
	{
		...
		echo "Failed to fetch students from the current section";
	}
	if($err_flag)
	{
		echo "<h1><center>Some errors encountered</center></h1>";
		echo "<table class=\"inner_table1\" width=\"500\" border=\"0\">";
		echo "<tr>";
		echo "<td>S.No</td>";
		echo "<td>Roll Number</td>";
		echo "<td>Error Encountered</td>";
		echo "</tr>";
		for($i=0;$i<$err_count;$i++)
		{
			...
			echo "<tr>";
			echo "<td>"."$x"."</td>";
			echo "<td>$err_rollnums[$i]</td>";
			echo "<td>$err_msg[$i]</td>";
			echo "</tr>";
		}
	}
	else
	{
		...
		echo "alert(\"Section updated electives\")";
		echo "</script>";
		echo "<h2><center>Updated All Details Successfully</center></h2>";
		header("refresh:10;url=msem.php");
	}
}
?>
