...
$err_flag=false;
	if(isset($_POST['submit']))
	{
		...
		$subject=strtoupper($_POST['sub_code']);
		$sp=$_POST['per_id'];
		$np=$_POST['back'];
		if(empty($date))
		{
			$err_flag=true;
			$err_msg[]="Missing Date";
		}
	  if($ymd<$date)
		{
			$err_flag=true;
			$err_msg[]="You Cannot enter attendence for future";
		}
		if(empty($year))
		{
			$err_flag=true;
			$err_msg[]="Missing Year";
		}
		if(empty($section))
		{
			$err_flag=true;
			$err_msg[]="Missing Section";
		}
		if(empty($subject))
		{
			$err_flag=true;
			$err_msg[]="Missing Subject";
		}
		if(empty($sp))
		{
			$err_flag=true;
			$err_msg[]="Missing Period ID";
		}
		if(!(isValidSubjectCode($subject)))
		{
			$err_flag=true;
			$err_msg[]="Invalid Subject Code";
		}
		if(!$err_flag)
		{
			$x=$year.",".$section.",".$subject;
			if(!isElective($subject))
			{
				header("location:enter_att1.php?flg=0&yearsecsub=$x&np=$np&sp=$sp&date=$date");
			}
			elseif(isElective($subject))
			{
				header("location:enter_attele1.php?flg=0&yearsecsub=$x&np=$np&sp=$sp&date=$date");
			}
		}
		else
		{
  ...
  <table width="500" border="0">
   <tr>
    ...
    <tr>
      ...
      <input type="text" name="sub_code" id="sub_code" /></td>
    </tr>
    <tr>
      <td>Enter Period</td>
      <td><label for="per_id"></label>
