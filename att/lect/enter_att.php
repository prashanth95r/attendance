...
if(function_exists('set_time_limit')) set_time_limit($maxtime);
	...
	{
		$subject_id=$det[2];
		if(isElective($subject_id))
		{
			header("location:enter_attele.php?flg=0&yearsecsub=$raw&np=$np&sp=$sp");
		}
	...
	if((!$isAlreadyEnteredFlag)&&isLiveSem($year))
	{
		if($flag==0)
		{
			...
			if($result)
			{
				...
				{
					echo "<center><form action=\"test.php\" id=\"form1\" name=\"myform\">";
					...
					if($x==0)
					{
						echo "No semeseter is live.So not Displaying students";
						exit();
					}
					if($x<5)
					{
				...
				{
					echo "currently no students exists in this section.contact admin for more details";
				}
			}
			else
			{
				echo "no such Section exists for these students.Please edit your schedule";
			}
		}
		elseif($flag==1)
		{	
				echo "<center><form action=\"test1.php\" id=\"form1\">";
				echo "<input type=\"radio\" name=\"year\" id=\"year\" value=\"$year\" style=\"display:none\" checked/>";
				echo "<input type=\"radio\" name=\"section\" id=\"section\" value=\"$section\" style=\"display:none\" checked/>";
				echo "<input type=\"radio\" name=\"period\" id=\"period\" value=\"$sp\" style=\"display:none\" checked/>";
				echo "<input type=\"radio\" name=\"noperiod\" id=\"noperiod\" value=\"$np\" style=\"display:none\" checked/><br/>";
