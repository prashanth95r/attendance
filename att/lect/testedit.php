...
if(!isset($_GET['year'])||!isset($_GET['section'])||!isset($_GET['period'])||!isset($_GET['date']))
{
	exit();
}
?>
...
<?php
	...
	$r=array();
	$transflag=true;
	$query="SELECT * FROM att_table WHERE date='$date' AND year='$year'";
	...
	if($result)
	{
		if(mysql_num_rows($result)>0)
		{
			...
			{
				if($row['section']==$section&&$row['period']==$period)
				{
					$uid=$row['uni_id'];
					$p_or_a=strtoupper($_GET[$uid]);
					if($p_or_a=="P")
					{
						$p_or_a=generatePresentee();
					}
					else
					{
						$p_or_a=generateAbsentee();
					}
					$query2="UPDATE att_table SET p_or_a=$p_or_a WHERE uni_id='$uid'";
		...
		{
			mysql_query("COMMIT");
			echo "<h1><center><b>Sucessfully updated attendence details</b></center></h1>";
			header("refresh:5;url=home.php");
		}
		elseif(!$transflag)
		{
			mysql_query("ROLLBACK");
			echo "<h1><center><b>Failed to update details</b></center></h1>";
			header("refresh:5;url=home.php");
