...
if(isset($_GET['errid']))
{
	...
	if($a==0)
	{
		echo "You Need to end ".romancrt($_GET['nsem']). "Semester before you create this semester";
	}
	elseif($a==1)
	{
		echo "unknown error";
	...
	elseif($a==5)
	{
		echo "Failed to exceute query";
	}
}
elseif(isset($_GET['sucid']))
{
	$a=$_GET['sucid'];
	if($a==1)
	{
		echo "<script type=\"text/javascript\">";
		echo "alert(\"Sucessfully started semester\")";
		echo "</script>";
		echo "<h2><center>Semester Creation Successfull</center></h2>";
	}
	elseif($a==2)
	{
		echo "<script type=\"text/javascript\">";
		echo "alert(\"sucessfully ended semester\")";
		echo "</script>";
		echo "<h2><center>Ended Semester Successfully</center></h2>";
	}
}
?>
<?php
...
$fac_branch=getDeptCode($username);
	...
	if($fac_branch!="CSE")
		$query="SELECT * FROM sem_table WHERE branch='$fac_branch' ORDER BY year,sem";
	else
		$query="SELECT * FROM sem_table WHERE branch='CSE' OR branch='MCSE' ORDER BY branch,year,sem";
	$result=mysql_query($query);
	if($result)
	{
		if(mysql_num_rows($result)>0)
		{
			echo "<table class=\"inner_table1\" width=\"500\" border=\"1\">";
			echo "<tr>";
			echo "<td>Year</td>";
			echo "<td>Branch</td>";
			echo "<td>Semester</td>";
			...
			while($row=mysql_fetch_array($result))
			{
				...
				if($row['status']==0)
				{
					echo "<td><a href=\"crt.php?id=0&year=$year&sem=$sem\">Start Semester</a></td>";
				}
				elseif($row['status']==1)
				{
					echo "<td><a href=\"crt.php?id=1&year=$year&sem=$sem\">End Semester</a></td>";
				}
				echo "</tr>";
			}
		}
		...
		{
			echo "No Sem exists.You need to <a href=\"../com/contactadmin.php\">contact admin</a>";
		}
	}
	else
	{
